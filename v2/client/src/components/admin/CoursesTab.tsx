  ...
  return (
    <div>
      ...
      <ul className="mt-2 space-y-2">
        {scoped.map(file => (
          <li key={file.id} className="flex items-center gap-2 rounded-xl bg-[#eef1f7] px-3 py-2 text-sm">
            <FileText className="h-4 w-4 shrink-0" /><a href={fileHref(file.fileKey)} target="_blank" rel="noreferrer" className="min-w-0 flex-1 truncate font-semibold text-[#14213d] hover:underline">{file.fileName}</a>
    ...
    <div>
      ...
      <div className="mt-2 space-y-4">
        {questions.map((item, qi) => (
          <div key={qi} className="rounded-2xl border border-[#dbe1ea] p-4">
            ...
            <div className="mt-3 space-y-2">
              {item.options.map((option, oi) => (
                <div key={oi} className="flex items-center gap-2">
                  <input type="radio" name={`c-${chapter.id}-${qi}`} aria-label={`Bonne réponse : option ${oi + 1}`} className="accent-[#14213d]" checked={item.correctOption === oi} onChange={() => patch(qi, { correctOption: oi })} />
                  ...
                  {item.options.length > 2 && <button type="button" aria-label="Retirer l'option" onClick={() => patch(qi, { options: item.options.filter((_, i) => i !== oi), correctOption: item.correctOption >= oi && item.correctOption > 0 ? item.correctOption - 1 : item.correctOption })}><X className="h-4 w-4 text-[#8a96ab]" /></button>}
                </div>
              ))}
              {item.options.length < 6 && <button type="button" onClick={() => patch(qi, { options: [...item.options, ""] })} className="text-xs font-semibold text-[#2f6fed]">+ Ajouter une option</button>}
            </div>
            <p className="mt-2 text-xs text-[#5b6b82]">Cochez le bouton radio de la bonne réponse.</p>
          </div>
        ))}
      </div>
...
function ChapterCard({ detail, chapter, onChange }: { detail: Detail; chapter: ChapterT; onChange: () => void }) {
  ...
  return (
    <div className="rounded-2xl border border-[#dbe1ea]">
      <div className="flex flex-wrap items-center gap-3 p-4">
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#eef1f8] text-sm font-bold text-[#14213d]">{chapter.position}</span>
        <div className="min-w-0 flex-1"><p className="truncate font-semibold text-[#14213d]">{chapter.title}</p><p className="text-xs text-[#5b6b82]">{Math.round(chapter.requiredSeconds / 60)} min · {chapter.quiz.length} question(s)</p></div>
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="block"><span className={label}>Temps total requis (minutes)</span><input className={input} type="number" min={1} value={form.minutes} onChange={event => setForm({ ...form, minutes: Number(event.target.value) })} /><span className="mt-1 block text-xs text-[#5b6b82]">Utilisé seulement si le cours n'a pas de chapitres.</span></label>
      ...
      <label className="flex items-center gap-3 text-sm font-semibold text-[#14213d]"><input type="checkbox" className="h-4 w-4 accent-[#14213d]" checked={form.isPublished} onChange={event => setForm({ ...form, isPublished: event.target.checked })} /> Publié (visible par les membres)</label>
...
function CourseManager({ id, onRemoved, onListChanged }: { id: number; onRemoved: () => void; onListChanged: () => void }) {
  ...
  if (detail.isLoading || !detail.data) return <Loader2 className="h-6 w-6 animate-spin text-[#2f6fed]" />;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title={`Chapitres (${data.chapters.length})`} subtitle="Le pourcentage de progression des membres est calculé à partir des chapitres validés.">
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-[#c9cec8] p-4"><h3 className="mb-3 font-display text-lg font-semibold text-[#14213d]">Ajouter un chapitre</h3><ChapterForm courseId={id} nextPosition={data.chapters.length + 1} onSaved={refresh} /></div>
      </Card>
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
      <Card title="Cours" actions={<button onClick={() => setSelected("new")} className={btnOrange + " !px-3 !py-1.5"}><Plus className="h-4 w-4" /> Nouveau</button>}>
        {list.isLoading && <Loader2 className="h-5 w-5 animate-spin text-[#2f6fed]" />}
        <ul className="space-y-2">
          {list.data?.map(course => (
            <li key={course.id}>
              <button onClick={() => setSelected(course.id)} className={`w-full rounded-2xl border p-3 text-left transition ${selected === course.id ? "border-[#14213d] bg-[#eef1f8]" : "border-[#dbe1ea] hover:border-[#9db3d6]"}`}>
                <span className="block font-semibold text-[#14213d]">{course.title}</span>
                <span className="mt-1 flex flex-wrap items-center gap-2 text-xs text-[#5b6b82]">{formatPrice(course.priceCents, course.currency)} · {course.chapterCount} chapitre(s) {course.isPublished ? <Badge color="green">Publié</Badge> : <Badge color="orange">Brouillon</Badge>}</span>
              </button>
            </li>
          ))}
        </ul>
      ...
      <div>
        ...
        {selected === null && <p className="rounded-2xl border border-dashed border-[#c9cec8] p-10 text-center text-[#5b6b82]">Sélectionnez un cours pour modifier son prix, son contenu, ses chapitres, ses quiz et ses fichiers, ou créez-en un nouveau.</p>}
      </div>
    </div>
  );
}
