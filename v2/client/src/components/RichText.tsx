  ...
  return (
    <div className="space-y-4 text-[1.02rem] leading-8 text-[#26313f]">
      {blocks.map((block, index) => {
        ...
        if (block.startsWith("## ")) return <h3 key={index} className="pt-2 font-display text-xl font-semibold text-[#14213d]">{inline(block.slice(3))}</h3>;
        if (block.startsWith("# ")) return <h2 key={index} className="pt-2 font-display text-2xl font-semibold text-[#14213d]">{inline(block.slice(2))}</h2>;
