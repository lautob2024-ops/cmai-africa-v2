  ...
  return (
    <div className="mt-4 border-t border-[#eeece4] pt-4">
      ...
      <ul className="space-y-3">
        {comments.data?.map(comment => (
          <li key={comment.id} className="flex gap-3">
            <span className="avatar-dot shrink-0 bg-[#193f36]">{initials(comment.authorName)}</span>
      ...
      <form onSubmit={(event: FormEvent) => { event.preventDefault(); send.mutate({ discussionId, body }); }} className="mt-4 flex gap-2">
        ...
        <button disabled={send.isPending} className="rounded-xl bg-[#193f36] px-5 text-sm font-semibold text-white disabled:opacity-60">Répondre</button>
      </form>
    </div>
  );
}
...
export default function Discussions() {
  ...
  return (
    <PageShell title="Discussions" kicker="Échanger" description="Posez une question, partagez une idée ou répondez aux autres membres.">
      <form onSubmit={(event: FormEvent) => { event.preventDefault(); create.mutate(form); }} className="rounded-3xl border border-[#e2dfd5] bg-white p-6">
        <h2 className="font-display text-xl font-semibold text-[#193f36]">Lancer une discussion</h2>
        ...
        <button disabled={create.isPending} className="mt-4 flex items-center gap-2 rounded-xl bg-[#eb6a3d] px-6 py-3 text-sm font-semibold text-white disabled:opacity-60">{create.isPending && <Loader2 className="h-4 w-4 animate-spin" />} Publier</button>
      </form>
      <div className="mt-8 space-y-4">
        {list.isLoading && <Loader2 className="h-6 w-6 animate-spin text-[#eb6a3d]" />}
        {list.data?.map(item => (
          <article key={item.id} className="rounded-3xl border border-[#e2dfd5] bg-white p-6">
            <p className="text-xs text-[#718078]">{item.authorName} · {formatDate(item.createdAt, true)}</p>
            <h3 className="mt-1 font-display text-xl font-semibold text-[#193f36]">{item.title}</h3>
            <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[#3f4d46]">{item.body}</p>
            <button onClick={() => setOpen(open === item.id ? null : item.id)} className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-[#eb6a3d]"><MessageSquare className="h-4 w-4" /> {open === item.id ? "Masquer les réponses" : "Voir / répondre"}</button>
