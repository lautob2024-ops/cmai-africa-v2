  ...
  return (
    <PageShell title="Paiement" kicker="Règlement" description="Payez votre cours par Mobile Money. Vous recevez ensuite automatiquement une quittance PDF par e-mail.">
      <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-3xl border border-[#e2dfd5] bg-white p-6 md:p-8">
          {outcome?.status === "confirmed" && (
            <div className="mb-6 rounded-2xl border border-[#bdd9c5] bg-[#edf4ee] p-5">
              <p className="flex items-center gap-2 font-semibold text-[#193f36]"><CheckCircle2 className="h-5 w-5 text-[#2e7d56]" /> Paiement confirmé — le cours est débloqué.</p>
              <div className="mt-3 flex flex-wrap gap-3">
                <Link href="/programmes" className="rounded-xl bg-[#193f36] px-4 py-2 text-sm font-semibold text-white">Commencer le cours</Link>
                {outcome.receiptKey && <a href={fileHref(outcome.receiptKey)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-[#193f36] px-4 py-2 text-sm font-semibold text-[#193f36]"><Download className="h-4 w-4" /> Quittance PDF</a>}
              </div>
            </div>
          )}
          ...
          {waiting ? (
            <div className="py-10 text-center" role="status" aria-live="polite">
              <Loader2 className="mx-auto h-10 w-10 animate-spin text-[#eb6a3d]" />
              <h2 className="mt-5 font-display text-2xl font-semibold text-[#193f36]">En attente de votre validation</h2>
              <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[#596961]">{waiting.message}</p>
              <p className="mt-2 text-xs text-[#718078]">Ne fermez pas cette page. Elle se met à jour dès que le paiement est confirmé.</p>
            </div>
          ) : payable.length === 0 && !courses.isLoading ? (
            <p className="py-8 text-center text-[#596961]">Aucun cours à payer pour le moment : vous avez déjà accès à tous les cours disponibles.</p>
          ) : (
            <form onSubmit={submit} className="space-y-6">
              ...
              <fieldset>
                ...
                <div className="mt-2 grid gap-3 sm:grid-cols-3">
                  {METHODS.map(item => (
                    <label key={item.id} className={`cursor-pointer rounded-2xl border p-4 text-sm transition ${method === item.id ? "border-[#193f36] bg-[#edf4ee]" : "border-[#e2dfd5] hover:border-[#9fb5a8]"}`}>
                      <input type="radio" name="method" className="sr-only" checked={method === item.id} onChange={() => setMethod(item.id)} />
                      <Smartphone className="h-5 w-5 text-[#193f36]" />
                      <span className="mt-2 block font-semibold text-[#193f36]">{item.label}</span>
                      <span className="mt-1 block text-xs text-[#718078]">{live ? item.hint : "Paiement manuel"}</span>
                    </label>
                  ))}
                </div>
              </fieldset>
              ...
              {live ? (
                <p className="rounded-xl bg-[#f3f1ea] p-4 text-sm leading-6 text-[#596961]">Après avoir cliqué, {method === "celtiis" ? "vous serez redirigé vers la page de paiement sécurisée." : "une notification s'ouvre automatiquement sur votre téléphone : saisissez votre code secret pour valider, sans revenir sur le site."}</p>
              ) : (
                <div className="rounded-2xl bg-[#193f36] p-5 text-sm text-white/90">
                  ...
                  <input className="form-input mt-3 !text-[#17231f]" placeholder="Référence de la transaction" value={manualRef} onChange={event => setManualRef(event.target.value)} />
              ...
              <button disabled={create.isPending || !selected} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#eb6a3d] px-6 py-4 text-sm font-semibold text-white transition hover:bg-[#d9572c] disabled:opacity-60">
                {create.isPending && <Loader2 className="h-4 w-4 animate-spin" />}
                {selected ? `Payer ${formatPrice(selected.priceCents, selected.currency)}` : "Payer"}
              </button>
              {selected && selected.currency !== "XOF" && live && <p className="text-center text-xs text-[#718078]">Le prélèvement Mobile Money est effectué en francs CFA (XOF), converti au taux du jour configuré par CMAI+Africa.</p>}
            </form>
          )}
        </section>

        <aside className="rounded-3xl border border-[#e2dfd5] bg-white p-6">
          <h2 className="font-display text-xl font-semibold text-[#193f36]">Mes paiements</h2>
          {history.data?.length === 0 && <p className="mt-4 text-sm text-[#718078]">Aucun paiement pour le moment.</p>}
          <ul className="mt-4 space-y-3">
            {history.data?.map(item => (
              <li key={item.id} className="rounded-2xl border border-[#eeece4] p-4 text-sm">
                <p className="font-semibold text-[#193f36]">{item.courseTitle ?? `Cours n°${item.courseId}`}</p>
                <p className="mt-1 text-xs text-[#718078]">{formatDate(item.paidAt ?? item.createdAt, true)} · {item.method.toUpperCase()}</p>
                <div className="mt-2 flex items-center justify-between">
                  <span className={`rounded-full px-3 py-1 text-xs font-bold ${item.status === "confirmed" ? "bg-[#edf4ee] text-[#2e7d56]" : item.status === "pending" ? "bg-[#fff8df] text-[#8a5a00]" : "bg-[#fff1e8] text-[#b8431c]"}`}>{STATUS_LABEL[item.status]}</span>
                  {item.receiptKey && <a href={fileHref(item.receiptKey)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-semibold text-[#193f36] hover:underline"><Download className="h-3.5 w-3.5" /> Quittance</a>}
                </div>
              </li>
            ))}
          </ul>
